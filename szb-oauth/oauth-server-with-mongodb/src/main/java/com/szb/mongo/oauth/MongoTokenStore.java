...
import com.szb.mongo.oauth.repositories.MongoOAuth2AccessTokenRepository;
import com.szb.mongo.oauth.repositories.MongoOAuth2RefreshTokenRepository;
...
public class MongoTokenStore implements TokenStore {
	...
	public Collection<OAuth2AccessToken> findTokensByClientIdAndUserName(String clientId, String userName) {
		...
		final Collection<MongoOAuth2AccessToken> noNullsTokens = oAuth2AccessTokens.stream().filter(byNotNulls())
				.collect(Collectors.toList());
		return noNullsTokens.stream().map(toOAuth2AccessToken()).collect(Collectors.toList());
	}

	private Predicate<MongoOAuth2AccessToken> byNotNulls() {
		return new Predicate<MongoOAuth2AccessToken>() {
			@Override
			public boolean test(final MongoOAuth2AccessToken token) {
				return token != null;
			}
		};
	}
	...
	public Collection<OAuth2AccessToken> findTokensByClientId(final String clientId) {
		...
		final Collection<MongoOAuth2AccessToken> noNullTokens = oAuth2AccessTokens.stream().filter(byNotNulls())
				.collect(Collectors.toList());
		return noNullTokens.stream().map(toOAuth2AccessToken()).collect(Collectors.toList());
	}
