    ...
    public static Object doFilter(ReflectiveMethodInvocation reflectiveMethodInvocation) throws Throwable {
        ...
        for (int i = 0; i < paramNumber; i++) {
            if (classType[i] == Predicate.class) {
                ...
                Path<BaseDocument> baseEntity;
                String queryDocumentName = (String)reflectiveMethodInvocation
                        .getUserAttribute(CustomAdvice.QUERY_DOCUMENT);
                baseEntity = Expressions.path(BaseDocument.class, queryDocumentName);
